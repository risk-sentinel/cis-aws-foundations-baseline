...
control 'C-2.1.2' do
  ...
  desc  'check', "
    ...
    - This returns a list of SCP/RCP policy IDs and names.
    ...
    - The output shows TargetId values representing accounts, OUs, or the root.
    ...
    The environment passes this recommendation if:
  ...
  "
  desc  'fix', "
    Design or confirm baseline guardrail SCPs.
  ...
  tag nist:                  ['AC-2 f', 'AC-2 (2)']
  tag nist_r4:               ['AC-2 (2)', 'AC-2 f']
  tag cci:                   ['CCI-000011', 'CCI-001682']
  ...
  # Dual-mode:
  # - When aws_organizations_role_arn is set, AssumeRole into the mgmt
  #   account and assert at least one customer-managed SCP exists. RCPs
  #   are checked when the policy type is enabled in the org (the
  #   resource emits a PolicyTypeNotEnabledException via connection_error
  #   when RCPs aren't enabled, which is reported but not failed —
  #   RCP-not-enabled is itself a finding that lands on C-2.1.2's broader
  #   "authorization guardrails" rationale).
  # - When the input is empty, stays attestation with an honest
  #   rationale that points the consumer at the dual-mode escape hatch.
  role_arn = input('aws_organizations_role_arn').to_s

  if role_arn.empty?
    describe 'Authorization guardrails on all Organization accounts (requires management context)' do
      skip 'attestation-required: SCP / RCP enumeration requires Organizations management or delegated-admin context. Set input aws_organizations_role_arn to a cross-account role for full automation; otherwise periodic-review attestation per profiles/cis-aws-foundations/attestations.example.json control_id C-2.1.2.'
    end
  else
    scps = aws_organizations_policies(role_arn: role_arn, policy_type: 'SERVICE_CONTROL_POLICY')

    if scps.connection_error
      describe 'Service Control Policy enumeration (requires aws_organizations_role_arn)' do
        it 'is callable via the cross-account role' do
          expect(scps.connection_error).to be_nil, scps.connection_error
        end
      end
    else
      describe 'Service Control Policies (customer-managed)' do
        it 'should include at least one customer-managed SCP (org has baseline guardrails)' do
          customer_managed = scps.rows.reject { |p| p[:aws_managed] }
          expect(customer_managed).not_to be_empty
        end
      end

      rcps = aws_organizations_policies(role_arn: role_arn, policy_type: 'RESOURCE_CONTROL_POLICY')

      if rcps.connection_error && rcps.connection_error.include?('PolicyTypeNotEnabled')
        describe 'Resource Control Policies (policy type not enabled in this org)' do
          it 'should be enabled (RCPs are a CIS 2.1.2 guardrail tier)' do
            expect(rcps.connection_error).to be_nil, rcps.connection_error
          end
        end
      elsif rcps.connection_error
        describe 'Resource Control Policy enumeration' do
          it 'is callable via the cross-account role' do
            expect(rcps.connection_error).to be_nil, rcps.connection_error
          end
        end
