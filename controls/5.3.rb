...
control 'C-5.3' do
  ...
  desc  'fix', "
    ...
    1. Create a metric filter based on the provided filter pattern that checks for 'root' account usage and uses the ` ` taken from audit step 1:
  ...
  tag ksi:                   ['KSI-CMT-LMC', 'KSI-CMT-RMV', 'KSI-IAM-APM', 'KSI-MLA-EVC', 'KSI-MLA-OSM', 'KSI-SVC-ACM']
  ...
  pattern = %q{{ $.userIdentity.type = "Root" && $.userIdentity.invokedBy NOT EXISTS && $.eventType != "AwsServiceEvent" }}
