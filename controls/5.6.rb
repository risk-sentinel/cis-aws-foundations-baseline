...
control 'C-5.6' do
  ...
  desc  'fix', "
    ...
    1. Create a metric filter based on the provided filter pattern that checks for AWS management Console login failures and uses the ` ` taken from audit step 1:
  ...
  pattern = %q{{ ($.eventName = ConsoleLogin) && ($.errorMessage = "Failed authentication") }}
