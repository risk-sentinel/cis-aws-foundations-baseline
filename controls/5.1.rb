...
control 'C-5.1' do
  ...
  desc  'fix', "
    ...
    1. Create a metric filter based on the provided filter pattern that checks for unauthorized API calls and uses the ` ` taken from audit step 1:

        ```
        aws logs put-metric-filter --log-group-name --filter-name --metric-transformations metricName=unauthorized_api_calls_metric,metricNamespace=CISBenchmark,metricValue=1 --filter-pattern \"{ ($.errorCode =\"*UnauthorizedOperation\") || ($.errorCode =\"AccessDenied*\") && ($.sourceIPAddress!=\"delivery.logs.amazonaws.com\") && ($.eventName!=\"HeadBucket\") }\"
        ```
  ...
  tag nist:                  ['IA-2 (2)', 'AC-2 (2)', 'AU-3 a', 'AU-3 d']
  tag ksi:                   ['KSI-IAM-AAM', 'KSI-IAM-APM', 'KSI-IAM-JIT', 'KSI-IAM-SNU', 'KSI-MLA-OSM']
  tag nist_r4:               ['AC-2 (2)', 'AU-3', 'IA-2 (2)']
  tag cci:                   ['CCI-000766', 'CCI-001682', 'CCI-000130', 'CCI-000133']
  tag cis_number:            '5.1'
  tag cis_rid:               '5.1'
  ...
  pattern = %q{{ ($.errorCode = "*UnauthorizedOperation") || ($.errorCode = "AccessDenied*") }}
