...
control 'C-5.9' do
  ...
  desc  'fix', "
    ...
    1. Create a metric filter based on the provided filter pattern that checks for AWS Configuration changes and uses the ` ` taken from audit step 1:
  ...
  tag nist:                  ['IA-2 (2)', 'AU-3 a', 'AU-3 d', 'CP-9 (a)']
  tag ksi:                   ['KSI-IAM-APM', 'KSI-MLA-OSM', 'KSI-RPL-ABO']
  tag nist_r4:               ['AU-3', 'CP-9 (a)', 'IA-2 (2)']
  tag cci:                   ['CCI-000766', 'CCI-000130', 'CCI-000133', 'CCI-000534']
  tag cis_number:            '5.9'
  tag cis_rid:               '5.9'
  ...
  pattern = %q{{ ($.eventSource = config.amazonaws.com) && (($.eventName = StopConfigurationRecorder) || ($.eventName = DeleteDeliveryChannel) || ($.eventName = PutDeliveryChannel) || ($.eventName = PutConfigurationRecorder)) }}
